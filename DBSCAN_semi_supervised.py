...
>>> for i in range(N_iterations): subsamples_matrix[i] = np.random.choice(data.shape[0], N_sub, replace = False)
...
>>> for i in range(N_iterations): y = db.fit_predict(data[subsamples_matrix[i]])
...
def load(hdf5_file_name, data, minPts, eps = None, quantile = 50, subsamples_matrix = None, core_points = None, 
metric = 'minkowski', p = 2, verbose = True):
    ...
    if core_points is None:
        ...
        data_weight = np.empty(data.shape[0])
        data_weight.fill(int(1))
        core_weight = np.empty(len(core_points))
        core_weight.fill(3)
        samples_weights = np.concatenate([data_weight, core_weight])
        data = np.concatenate((data, np.array(core_points)),0)
...
def DBSCAN(data, minPts, eps = None, quantile = 50, subsamples_matrix = None, core_points= None, 
metric = 'minkowski', p = 2, verbose = True):
    ...
    with NamedTemporaryFile('w', suffix = '.h5', delete = True, dir = './') as f:
        eps = load(f.name, data, minPts, eps, quantile, subsamples_matrix, core_points, metric, p, verbose)
